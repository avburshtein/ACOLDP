...
**Дата отчёта.** NEVER угадывать дату и NEVER подставлять текущую/предполагаемую. Допустимый источник даты — только явный надёжный timestamp во входных данных: дата коммита, timestamp строки лога, заголовок сообщения, имя файла-экспорта. Если источника нет — в метаданных пишется "Дата: не определена ❓", и это корректный результат.
...
### 📅 Метаданные дня
```
- Дата: YYYY-MM-DD | не определена ❓ (нет надёжного timestamp во входе)
